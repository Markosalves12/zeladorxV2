...
def gantt_jardinagem(request, userid):
    ...
    tipos = [
        {'nome': 'Gantt de serviços', 'link': ''},
    ]

    if setores['habilitar_jardinagem_secundaria'] and setores['habilitar_jardinagem']:
        tipos.insert(1, {'nome': 'Jardinagem', 'link': reverse('gantt_jardinagem', kwargs={'userid': userid})})
    else:
        return redirect('gantt_limpeza_predial', userid)

    if setores['habilitar_limpeza_secundaria'] and setores['habilitar_limpeza']:
        tipos.insert(2, {'nome': 'Limpeza predial', 'link': reverse('gantt_limpeza_predial', kwargs={'userid': userid})})
    ...
    return render(
        request=request,
        template_name='agendamentos/gantt.html',
        context={
            'formatted_events': formatted_events,  # Convertendo para JSON
            'app_name': 'Gantt Jardinagem',
            'link_tipos': tipos,
